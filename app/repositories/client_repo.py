...
class AsyncCollection(Protocol):
    ...
    Note: **kwargs are intentionally omitted from every method.
    Motor uses explicit named parameters (no **kwargs), and the
    repository never forwards extra keyword arguments — keeping the
    protocol signatures narrow ensures both Motor and mongomock-motor
    satisfy structural subtyping without type-checker complaints.
    """

    def insert_one(self, document: dict) -> Any: ...
    def find_one(self, filter: Any = None) -> Any: ...
    def find(self, filter: Any = None) -> Any: ...
    def replace_one(self, filter: Any, replacement: dict) -> Any: ...
    def update_one(self, filter: Any, update: Any) -> Any: ...
    def delete_one(self, filter: Any) -> Any: ...
